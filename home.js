(function(){
  ...
  async function loadFeed(){
    ...
    try{
      ...
      if(!data?.length){
        host.innerHTML='<div class="feed-card"><p class="feed-content">校墙还很安静，成为第一个分享校园故事的人吧。</p><a class="text-link" href="messages.html">发布第一条留言 →</a></div>';
        return;
      }
      host.innerHTML=data.map(item=>`<article class="feed-card"><div class="feed-meta"><span class="feed-author"><span class="feed-avatar">${escapeHtml(item.username==='匿名用户'?'匿':item.username?.slice(0,1)||'I')}</span>${escapeHtml(item.username||'ISAer')}</span><time datetime="${escapeHtml(item.created_at)}">${relativeTime(item.created_at)}</time></div><p class="feed-content">${escapeHtml(item.content)}</p><a class="text-link" href="details.html?id=${encodeURIComponent(item.id)}">进入讨论 →</a></article>`).join('');
    }catch(error){
      console.error("loadFeed 异常：", error);
      host.innerHTML='<div class="feed-card"><p class="feed-content">暂时无法同步最新动态，你仍可以进入校墙查看或发布内容。</p><a class="text-link" href="messages.html">前往校墙 →</a></div>';
    }
  }
